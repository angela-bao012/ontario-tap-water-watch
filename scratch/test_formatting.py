...
def make_display_name(raw_name):
    ...
    # e.g., "WELL SUPPLY" -> "WELL"
    # We do a case-insensitive regex swap with word boundaries
...
def test():
    ...
    # Query locations that contain "OF" or "AND" or "SCHOOL" or "WELL" or hyphens
    cur.execute("""
        SELECT DISTINCT location 
        FROM water_quality_data 
        WHERE location IS NOT NULL 
          AND (location LIKE '% OF %' OR location LIKE '% AND %' OR location LIKE '%SCHOOL%' OR location LIKE '%WELL%' OR location LIKE '% - %')
        LIMIT 40;
    """)
    rows = cur.fetchall()
    ...
    for row in rows:
        loc = row[0]
        print(f"  Raw: {loc!r} -> Display: {make_display_name(loc)!r}")
